...
def data_generator_yellow(N,channel):
    data=[]
    total_hist=0
    channel_r=2
    channel_g=1
    for i in range(N+1):
        ...
        for i in range (len(image)):
            for j in range (len(image[0])):
                if (int(image[i,j,channel_r])>50 and int(image[i,j,channel_g])>50):
                    input=(image[i,j,channel_r]+image[i,j,channel_g])/2
...
data=data_generator_yellow(37,1)
#print(len(data))
#print(data)
